			...
			if(ref_verify_crc8(data,JUDGE_HEAD_LENGTH))
			{
				...
				if(ref_verify_crc16(data,data_fulllength))
				{
					*data += 1;
					...
					{
						case 0x0003: //血量读取
						HP.red_1_robot_HP=data[7+1]<<8 | data[7];
						HP.blue_1_robot_HP=data[7+17]<<8 | data[7+16];
					break;
					case 0x0101:
					...
					break;
					case 0x0208://剩余弹量获取
						remain.bullet_remaining_num_17mm=data[7+2+1] << 8 | data[7+2];
					break;
					}
...
{
	return (power.chassis_power_buffer);
}
...
{
	return power.shooter_id1_42mm_cooling_heat;
}
...
/**
  ...
  * @attention  实时热量
  */
float JUDGE_usGetSpeedHeat42(void)
{
...
{
	return My_status.shooter_id1_42mm_cooling_limit;
}
...
{
	return My_status.shooter_id1_42mm_cooling_rate;
}
