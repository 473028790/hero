...
{
	ranging_distance_char1[0]=(((uint8_t)(ranging_distance/1000))%10)+0x30;/*0x30是ASCll表对应的数字0*/
...
static void Char_Init()
{
	...
	else if(UI_char.char_send_times==10){/*添加 发射机构电的字符串*/
	...
	else if(UI_char.char_send_times==11){/*添加 发射机构电的字符串*/
	...
	else if(UI_char.char_send_times==12){/*添加 发射机构电的字符串*/
	...
	else if(UI_char.char_send_times==13){/*更新 超级电容剩余量百分十位*/
	...
	else if(UI_char.char_send_times==14){/*更新 超级电容余量百分个位*/
	Get_ranging_distance();
