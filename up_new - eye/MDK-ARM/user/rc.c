...
void ReadRc_Gimbal(void)
{
	...
	if(STOP==1)
	{
		...
		if(IMU_first==1)	yaw_outer_pid.target=last_store[2];
		yaw_outer_pid.target+=(degree_k*(-left_X));
		pitch_outer_pid.target+=(degree_k*(left_Y));
		/*
		if(MODE==3 || MODE==1)
		{
				yaw_outer_pid.target-=vision_info.aimbot_rec_data.aimbot_yaw_angle;
				pitch_outer_pid.target+=vision_info.aimbot_rec_data.aimbot_pit_angle;
		}
		*/
...
{
	infra_red_GPIO=HAL_GPIO_ReadPin(GPIOC,GPIO_PIN_6);
