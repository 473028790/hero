...
void friction_PID_init()   //摩擦轮PID初始化
{
   int motor=0;
      ...
      Friction_motor[0].kp=8.1;
      Friction_motor[0].ki=0.04;
      Friction_motor[0].kd=0;

      Friction_motor[1].kp=8.0;
      Friction_motor[1].ki=0.01;
...
{
	dial_mode=dial_mode_choose;//模式选择
...
void Gimbal_outer_PID(struct gimbal_outer_pid *motor) //云台外环位置式PID专用
{
   ...
   if(motor->judge==3)
   {
   if(motor->out>=7000)   motor->out=7000;
   if(motor->out<=-7000)  motor->out=-7000;
   }
}
