...
#define front_degree 6838.0f    //当yaw朝前的时候，yaw电机的ecd
#define cheassis_follow_speed 500.00f

#define cheassis_follow 0
#define Small_gyroscope 1
#define separate_mode   2
int chassis_mode=separate_mode;
int Q_sign=0;
int E_sign=0;
...
void Function_Choose(void)
{
	...
	{
		if(MODE==1)  CtoG();   					//底盘跟随
		else if(MODE==3)  separate(R_X,R_Y);	//分离控制
		else if(MODE==2)  Circle(R_X,R_Y);		//小陀螺
	}
	if(STOP==2)
	{
		...
		else if(chassis_mode==cheassis_follow)
		{
			CtoG();   											//底盘跟随
		}
		else if(chassis_mode==separate_mode)
		{
			separate(KEY_Date.R_X,KEY_Date.R_Y);
		}
	}
}
/*************************************底盘跟随**************************************/
...
void Circle(int16_t right_X,int16_t right_Y)
{
		...
		wheel_moter[0].target_speed=500;   //旋转量
		wheel_moter[1].target_speed=500;
		wheel_moter[2].target_speed=500;
		wheel_moter[3].target_speed=500;
