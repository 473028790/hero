...
#define front_degree 1677.0f    //当yaw朝前的时候，yaw电机的ecd
...
void CtoG()
{
	...
	if(Is_finishctog==1)
	{
		if(ecd_diff>=150||ecd_diff<=-150)    Is_finishctog=0; //判断云台是否转动和底盘是否开始跟随
	}
	if(ecd_diff<=150&&ecd_diff>=-150) Is_finishctog=1;         //如果机械角度靠近开始跟随时的机械角度，则完成跟随

	if(Is_finishctog==0)      //底盘开始跟随，则进入调整
	{
		if(ecd_diff>=1000) ecd_diff=1000;         //跟随速度限幅
		else if(ecd_diff <=-1000) ecd_diff=-1000;
		
		wheel_moter[0].target_speed+=-((ecd_diff/cheassis_follow_speed)*2800.0f);
		wheel_moter[1].target_speed+=-((ecd_diff/cheassis_follow_speed)*2800.0f);
		wheel_moter[2].target_speed+=-((ecd_diff/cheassis_follow_speed)*2800.0f);
		wheel_moter[3].target_speed+=-((ecd_diff/cheassis_follow_speed)*2800.0f);
	}
}
/*************************************小陀螺**************************************/	
