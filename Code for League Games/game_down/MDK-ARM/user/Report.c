...
{	
		...
		for(h = 0; h < 5; h++ )
		{
			for(m = 0; m < 4; m++ )
			{
				HAL_UART_Transmit(&huart1,(uint8_t *)&vofa_data[h].sendbuf[m],sizeof(vofa_data[h].sendbuf[m]),0x10);
			}
		}
		HAL_UART_Transmit(&huart5,(uint8_t *)&tail_3,1,0x10);
		HAL_UART_Transmit(&huart4,(uint8_t *)&tail_3,1,0x10);
	
		HAL_UART_Transmit(&huart3,(uint8_t *)&tail,1,0x10);
		
		HAL_UART_Transmit(&huart1,(uint8_t *)&tail_2,1,0x10);
};
