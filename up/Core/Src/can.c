...
union
{
	unsigned char RC_buffer[RC_FRAME_LENGTH];
	uint8_t sendbuf[3];
}RC_data;
...
{
	CAN_TxHeaderTypeDef tx;
	memcpy(&RC_data.RC_buffer,pData,18);
	...
	static uint8_t Data[8];
	Data[0]=RC_data.sendbuf[0];

	Data[1]=RC_data.sendbuf[1];
	Data[2]=RC_data.sendbuf[2];
	Data[3]=10;
	Data[4]=0;
	Data[5]=0;
	Data[6]=0;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  CAN_RxHeaderTypeDef CAN2_Rx;
	...
	if(hcan->Instance==CAN2)   //CAN2
	{
      ...
      break;
			case 0x300:
			cnt50=(float)((int16_t)(Data[0]<<8|Data[1]));
      break;
			default: {break;}
		}
	}
