...
uint16_t USART1_Data_Len = 0;
uint8_t USART1_Flag=0;
uint8_t USART1_Rx_temp[2][REFFER_REC_LENGTH] = {0};
...
void Usart_Init_Report(void)
{
	...
	HAL_UART_Receive_DMA(&huart1, (uint8_t*)USART1_Rx_temp[USART1_Flag], REFFER_REC_LENGTH); 
};
...
{ 
	...
	if(huart->Instance==MPU_USART)
	{
		...
		HAL_UART_Receive_IT(&huart5,&MPU_data,1);
	}
...
{
  if(huart == &huart1)//裁判系统
{
    __HAL_UART_CLEAR_IDLEFLAG(&huart1);
    /*your own code*/
    HAL_UART_DMAStop(&huart1);  
    USART1_Data_Len = REFFER_REC_LENGTH - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
//			for(uint8_t i=0;i<USART3_Data_Len;i++)
//			{
////				USART3_Rx_Buff[USART3_Flag][i]=USART3_Rx_temp[i];
//				USART3_Rx_temp[i]=0;
//			};
  judge_rcdata(USART1_Rx_temp[USART1_Flag],USART1_Data_Len);
  if(USART1_Flag)USART1_Flag=0;
  else USART1_Flag=1;
  HAL_UART_Receive_DMA(&huart5, (uint8_t*)USART1_Rx_temp[USART1_Flag], REFFER_REC_LENGTH);                         
}
}
/* USER CODE END 1 */
